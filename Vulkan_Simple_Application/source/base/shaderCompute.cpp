...
private:
	void initWindow()
	{
		...
		glfwSetFramebufferSizeCallback(window, [](GLFWwindow* window, int width, int height) {
			auto app = static_cast<ComputeShader*>(glfwGetWindowUserPointer(window));
	...
	void createLogicalDevice()
	{
		...
		{
			.queueFamilyIndex = graphicsAndComputeIndex,
			.queueCount = 1,
	...
	{
		vk::raii::ShaderModule shaderModule = createShaderModule(readFile("resources/shaders/compute/slang_compute.spv"));
	...
	void createCommandPool()
	{
		...
		commandPool = vk::raii::CommandPool(device, nullptr);
	}
	...
	void createShaderStorageBuffers()
	{
		...
		for (auto& partice : particles)
		{
			...
			partice.position = glm::vec2(x, y);
			partice.velocity = normalize(glm::vec2(x, y)) * 0.00025f;
			partice.color = glm::vec4(rndDist(rndEngine), rndDist(rndEngine), rndDist(rndEngine), 1.0f);
		}
	...
	void createComputrDescriptorSets()
	{
	...
	void drawFrame()
	{
...

		{
			...
			computeQueue.submit(graphicsSubmitInfo, nullptr);
	...
	static VKAPI_ATTR vk::Bool32 VKAPI_ATTR debugCallback(vk::DebugUtilsMessageSeverityFlagBitsEXT severity, vk::DebugUtilsMessageTypeFlagsEXT type, const vk::DebugUtilsMessengerCallbackDataEXT* pCallbackData, void*)
	{
